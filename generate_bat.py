...
def get_batch_content():
    ...
    lines.append(f"echo {C_GOLD}:{C_RESET}   {C_GOLD}╚═╝     ╚═╝ ╚═════╝ ╚═════╝ ╚═╝╚══════╝╚══════╝    ╚═╝  ╚═╝ ╚═════╝ ╚═════╝╚══════╝╚══════╝╚══════╝{C_RESET}")
    lines.append(f"echo {C_GOLD}:{C_RESET}")
    lines.append(f"echo {C_GOLD}:{C_RESET}                         {C_GREY}Access from Phone or Any Device via HTTPS{C_RESET}")
    lines.append(f"echo {C_GOLD}:{C_RESET}")
    lines.append(f"echo {make_bar('=', TERM_WIDTH, C_GOLD)}")
    lines.append(f"echo.")
    lines.append(f"echo   {C_AMBER}Starting with Share Mode enabled...{C_RESET}")
    lines.append(f"echo   {C_GREY}A public HTTPS URL will be generated for remote access.{C_RESET}")
    lines.append(f"echo   {C_GREY}You can access this URL from your phone or any device.{C_RESET}")
    lines.append(f"echo.")
    lines.append(f"echo {make_bar('-', TERM_WIDTH, C_GREY)}")
    lines.append(f"echo.")
    lines.append('if not exist "recordings" mkdir recordings')
